...
The application uses environment variables for configuration. Create a `.env` file in the `backend/` directory using the provided template.
...
2. **Customize the values** (especially `JWT_SECRET` for production)

3. **The application will work without `.env`** - it uses sensible defaults
